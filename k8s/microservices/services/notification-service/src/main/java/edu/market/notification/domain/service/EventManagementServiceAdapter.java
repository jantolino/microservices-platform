...
public class EventManagementServiceAdapter implements EventManagementServicePort {
    ...
    /**
     * Extrae el ID de notificación del payload de un evento.
     *
     ...
     */
    @Override
    public UUID extractEventFromPayload(String payload, EventType eventType) {
        ...
        try {
            ...
            // Estrategia de deserialización basada en el tipo de evento
            switch (eventType) {
                case NOTIFICATION_SCHEDULED:
                    log.debug("extractEventFromPayload - Deserializing to NotificationScheduledEvent");
                    NotificationScheduledEvent event = serialization.deserialize(payload, NotificationScheduledEvent.class);
                    log.debug("extractEventFromPayload - Extracted ID: {}", event.getNotificationId());
                    return event.getNotificationId();
                    
                case NOTIFICATION_SENT:
                    log.debug("extractEventFromPayload - Deserializing to NotificationSentEvent");
                    NotificationSentEvent eventSent = serialization.deserialize(payload, NotificationSentEvent.class);
                    log.debug("extractEventFromPayload - Extracted ID: {}", eventSent.getNotificationId());
                    return eventSent.getNotificationId();
                    
                case NOTIFICATION_FAILED:
                    log.debug("extractEventFromPayload - Deserializing to NotificationFailedEvent");
                    NotificationFailedEvent eventFailed = serialization.deserialize(payload, NotificationFailedEvent.class);
                    log.debug("extractEventFromPayload - Extracted ID: {}", eventFailed.getNotificationId());                    
                    return eventFailed.getNotificationId();
                    
                case NOTIFICATION_CANCELED:
                    log.debug("extractEventFromPayload - Deserializing to NotificationCanceledEvent");
                    NotificationCanceledEvent eventCanceled = serialization.deserialize(payload, NotificationCanceledEvent.class);
                    log.debug("extractEventFromPayload - Extracted ID: {}", eventCanceled.getNotificationId());
                    return eventCanceled.getNotificationId();
                    
                default:
                    // Para tipos de eventos no manejados específicamente, usar enfoque genérico
                    log.debug("extractEventFromPayload - Using generic approach for event type: {}", eventType);
                    UUID result = extractIdFromGenericPayload(payload);                    
                    return result;
            }
        } catch (Exception e) {
            log.warn("extractEventFromPayload - Error extracting notification ID from payload: {}", e.getMessage());
            return null;
        }
    }
    
    /**
     * Extrae el ID de notificación de un payload genérico usando un Map.
    ...
    private UUID extractIdFromGenericPayload(String payload) {
        ...
        try {
            ...
            // Intentar con el campo notificationId primero
            if (payloadMap.containsKey("notificationId") && payloadMap.get("notificationId") != null) {
                String idStr = payloadMap.get("notificationId").toString();
                UUID id = UUID.fromString(idStr);
                log.debug("extractIdFromGenericPayload - ID extracted from notificationId field: {}", id);                
                return id;
            }
            
            log.debug("extractIdFromGenericPayload - Could not extract notification ID from payload");            
            return null;
        } catch (Exception e) {
            log.warn("extractIdFromGenericPayload - Error extracting notification ID from generic payload: {}", e.getMessage());            
            return null;
        }
    }
    ...
    public LocalDateTime extractScheduledTimeFromPayload(String payload) {
        ...
        try {
            // Deserializar directamente a NotificationScheduledEvent ya que es el único evento que usa SCHEDULED
            log.debug("extractScheduledTimeFromPayload - Deserializing payload to NotificationScheduledEvent");
            NotificationScheduledEvent scheduledEvent = serialization.deserialize(payload, NotificationScheduledEvent.class);
            if (scheduledEvent != null && scheduledEvent.getScheduledTime() != null) {
                log.debug("extractScheduledTimeFromPayload - Scheduled date extracted from event: {}", scheduledEvent.getScheduledTime());                
                return scheduledEvent.getScheduledTime();
            }
